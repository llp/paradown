...
- `libtorrent` 目前已有主 crate API、adapter 包、可编译的 native CXX bridge、torrent-file metadata 提取、magnet metadata exchange、完整控制入口、fast-resume 状态闭环、本地 peer-wire 端到端验证、native CLI、torrent/magnet discovery、swarm provider layer、tracker-list 缓存、tracker/web-seed/peer 注入、超时诊断、独立 release 打包策略和公网 smoke 入口；下一步是更系统的真实公网样本验证和可选外部索引 provider
