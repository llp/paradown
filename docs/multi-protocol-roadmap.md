...
- P2P 已经开始按完整 libtorrent 集成路线推进：
  ...
  - P6 swarm provider layer 已接入主 torrent 启动流程，支持 magnet hints、静态 tracker/peer/web-seed、带 TTL 缓存的远程 tracker list、HTML/feed/text discovery、候选限量和 provider 诊断
  - native adapter 已有公网 smoke 和 soak matrix 脚本入口，支持 tracker 注入、provider cache、discovery 输入、超时诊断和样本 summary，不进入默认 CI
...
- `libtorrent` 目前已有主 crate API、adapter 包、可编译的 native CXX bridge、torrent-file metadata 提取、magnet metadata exchange、完整控制入口、fast-resume 状态闭环、本地 peer-wire 端到端验证、native CLI、torrent/magnet discovery、swarm provider layer、tracker-list 缓存、tracker/web-seed/peer 注入、超时诊断、独立 release 打包策略、公网 smoke/soak 入口和 release readiness gate；下一步是更系统地积累真实公网样本结果和可选外部索引 provider
